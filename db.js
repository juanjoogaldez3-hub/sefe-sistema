...
function mapClienteFromDB(c){
  return {
    ...
    direccionEntrega:c.direccion_entrega, horaLimiteEntrega:c.hora_limite_entrega||'', ruta:c.ruta||'', fechaAlta:c.fecha_alta,
...
function mapProductoFromDB(p){
  return {
    id:p.id, codigo:p.codigo, codigoBarras:p.codigo_barras||'', nombre:p.nombre, precio:Number(p.precio),
...
function mapDocumentoFromDB(d, todosAbonos){
  ...
  return {
    ...
    entregaInfo:d.entrega_info, paraDespacho:d.para_despacho===true, preparacion:d.preparacion||null, anulado:d.anulado, motivoAnulacion:d.motivo_anulacion,
...
async function guardarCliente(cli){
  const row = {
    ...
    direccion_entrega:cli.direccionEntrega, hora_limite_entrega:cli.horaLimiteEntrega||null, ruta:cli.ruta||null, fecha_alta:cli.fechaAlta,
...
async function guardarDocumento(d){
  const row = {
    ...
    entrega_info:d.entregaInfo, para_despacho:d.paraDespacho===true, preparacion:d.preparacion||null, anulado:d.anulado, motivo_anulacion:d.motivoAnulacion,
...
async function guardarProducto(p){
  const row = {
    codigo:p.codigo, codigo_barras:p.codigoBarras||null, nombre:p.nombre, precio:p.precio, costo:p.costo,
