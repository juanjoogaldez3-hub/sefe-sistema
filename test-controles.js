...
ok('la gasolina NO toca Bancos (sin cuenta de pago ni póliza)', !/origen:'gasolina'/.test(src) && !/_gasRegistrarMov\(/.test(src) && !/_gasAnularMov\(/.test(src) && !/id="gas-cta"/.test(html) && !/id="gas-cta"/.test(src));
ok('la pantalla de gasolina no muestra columna Póliza', !/<th>Póliza<\/th>/.test(html));
